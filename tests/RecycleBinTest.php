...
class RecycleBinTest extends TestCase
{
    ...
    {
        $page = Page::query()->first();
        $book = Recipe::query()->where('id', '!=', $page->book_id)->whereHas('pages')->whereHas('chapters')->with(['pages', 'chapters'])->firstOrFail();
        $editor = $this->getEditor();
        $this->actingAs($editor)->delete($page->getUrl());
        $this->actingAs($editor)->delete($book->getUrl());
    ...
    {
        /** @var Entity $entity */
        foreach ([new Bookshelf(), new Recipe(), new Chapter(), new Page()] as $entity) {
